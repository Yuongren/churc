...
async function loadServicesMedia() {
    const gallery = document.getElementById("services-media-gallery");
    ...
    try {
        const response = await fetch(`${window.API_BASE}/api/media/section/services`);
        if (!response.ok) throw new Error("Failed to load services media");

        const items = await response.json();

        if (!items || items.length === 0) {
            gallery.innerHTML = `<div class="empty-media">No services media uploaded yet.</div>`;
            return;
        }
